...
try:
    url = f"https://api.zoom.us/v2/rooms/calendar/services/{id}/resources/{json_data['basic']['calendar_resource_id']}"
    response = requests.request("GET", url, headers=headers)
...
try:
    resp = requests.post( 
        f"https://api.zoom.us/v2/rooms/calendar/services/{id}/resources",
            headers=headers,
            json = req
        )
